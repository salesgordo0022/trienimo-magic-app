      ...
      <main className="max-w-5xl mx-auto p-4 grid gap-4 md:grid-cols-[280px_1fr]">
        <aside className="space-y-3">
          ...
          <div className="bg-white border border-black/10">
            <div className="bg-[var(--yellow)] px-3 py-2 font-display font-black uppercase text-sm">
              Alunos
            </div>
            {students.length === 0 && (
              <div className="p-3 text-xs text-gray-500">Nenhum aluno ainda. Cadastre acima.</div>
            )}
            {students.map((s) => (
              <button
                key={s.id}
                onClick={() => setSelected(s.id)}
                className={`w-full text-left px-3 py-2 text-sm border-t ${selected === s.id ? "bg-black text-[var(--yellow)]" : "hover:bg-gray-50"}`}
              >
                {s.nome ?? "(sem nome)"}
              </button>
            ))}
        ...
        <section>
          {selected ? (
            <StudentWorkouts
              studentId={selected}
              studentName={students.find((s) => s.id === selected)?.nome ?? ""}
            />
          ) : (
            <div className="bg-white border p-8 text-center text-sm text-gray-500">
              Selecione um aluno.
            </div>
          )}
        </section>
...
function NewStudentForm() {
  ...
  return (
    <div className="bg-white border border-black/10">
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full bg-black text-[var(--yellow)] px-3 py-2 font-display font-black uppercase text-sm text-left flex items-center gap-2"
      >
        <Plus className="w-4 h-4" />
        {open ? "Fechar" : "Novo aluno"}
      </button>
      {open && (
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (!nome || !email || password.length < 6) return;
            create.mutate({ data: { nome, email, password } });
          }}
        ...
        >
          ...
          <Button
            ...
            className="w-full bg-[var(--yellow)] text-black font-bold uppercase"
          >
            Cadastrar aluno
          </Button>
          <p className="text-[10px] text-gray-500">
            O aluno usará esse e-mail e senha para entrar. Já ficará vinculado a você.
          </p>
        </form>
      )}
    </div>
...
function StudentWorkouts({ studentId, studentName }: { studentId: string; studentName: string }) {
  ...
  return (
    <div className="bg-white border border-black/10">
      <div className="bg-[var(--yellow)] px-3 py-2 font-display font-black uppercase text-sm">
        Fichas de {studentName}
      </div>
      <form
        onSubmit={(e) => {
          e.preventDefault();
          if (!letra) return;
          create.mutate({ data: { letra, assigned_to: studentId } });
        }}
        ...
        <ul className="divide-y">
          {workouts.map((w) => (
            <li key={w.id} className="flex items-center gap-3 p-3">
              <div className="bg-black text-[var(--yellow)] w-14 h-14 flex items-center justify-center font-display font-black text-3xl">
                {w.letra}
              </div>
              ...
              <Link
                to="/ficha/$id"
                params={{ id: w.id }}
                className="inline-flex items-center gap-1 bg-gray-100 px-2 py-1 text-xs font-bold uppercase"
              >
              ...
              <Link
                ...
                params={{ id: w.id }}
                search={{ tab: "executar" }}
                className="inline-flex items-center gap-1 bg-[var(--yellow)] text-black px-2 py-1 text-xs font-bold uppercase"
              >
              ...
              <button
                onClick={() => {
                  if (confirm("Excluir?")) del.mutate({ data: { id: w.id } });
                }}
