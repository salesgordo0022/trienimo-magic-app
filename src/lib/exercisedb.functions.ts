...
export const EXERCISE_NAME_PT: Record<string, string> = {
  ...
  "alternating dumbbell curl": "rosca alternada com halteres",
  ...
  "twenty ones": "vinte e um",
  "incline dumbbell press": "supino inclinado com halteres",
  "neutral grip dumbbell press": "supino pegada neutra com halteres",
  "svend press": "supino svend",
  "plate press": "supino com anilha",
  "band bench press": "supino com elástico",
  ...
  "sled pull": "puxada no sled",
  "tire flip": "virar pneu",
  "battle rope": "corda naval",
  "battle rope slam": "jogada de corda naval",
  "battle rope wave": "onda de corda naval",
  "weighted vest walk": "caminhada com colete",
  "sandbag squat": "agachamento com saco de areia",
  "sandbag deadlift": "levantamento terra com saco de areia",
  "sandbag carry": "carregar saco de areia",
  "sandbag clean": "clean com saco de areia",
  "sandbag shoulder": "ombro com saco de areia",
};
...
// ─── Batch translate: catálogo inteiro (exercises_catalog) ───
export const translateAllCatalogExercises = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    const { data: role } = await context.supabase.rpc("has_role", { _user_id: context.userId, _role: "admin" });
    if (!role) throw new Error("Apenas admin pode traduzir");
    const db = await dbClient();
    const { data: rows } = await db.from("exercises_catalog").select("*");
    if (!rows) return { total: 0, ok: 0, fail: 0 };
    let ok = 0, fail = 0;
    for (const row of rows) {
      try {
        const pt = await translateEN(row.name);
        if (pt !== row.name) {
          await db.from("exercises_catalog").update({ name_pt: pt }).eq("id", row.id);
        }
        if (row.instructions?.length) {
          const translated = await Promise.all(row.instructions.map((s: string) => translateEN(s)));
          await db.from("exercises_catalog").update({ instructions_pt: translated }).eq("id", row.id);
        }
        ok++;
      } catch { fail++; }
    }
...
// ─── Batch translate: todos os exercícios já nas fichas (public.exercises) ───
export const translateAllWorkoutExercises = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    const { data: role } = await context.supabase.rpc("has_role", { _user_id: context.userId, _role: "admin" });
    if (!role) throw new Error("Apenas admin pode traduzir");
    const db = await dbClient();
    const { data: rows } = await db.from("exercises").select("id, nome");
    if (!rows) return { total: 0, ok: 0, fail: 0 };
    let ok = 0, fail = 0;
    for (const row of rows) {
      try {
        const translated = await translateEN(row.nome);
        if (translated !== row.nome) {
          await db.from("exercises").update({ nome: translated }).eq("id", row.id);
        }
        ok++;
      } catch { fail++; }
    }
