...
DROP POLICY IF EXISTS "exercises_catalog readable by all" ON public.exercises_catalog;
...
DROP POLICY IF EXISTS "authenticated can read exercise gifs" ON storage.objects;
...
DROP POLICY IF EXISTS "messages select open" ON public.messages;
...
DROP POLICY IF EXISTS "messages update open" ON public.messages;
